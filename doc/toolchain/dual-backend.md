...
- Core I/O builtins — `open`, `with`, file methods, `open_mmap`, `import json`, `input()`, `clock()`, collection methods, **full string methods**, **`break` / `continue`**, **`raise` / `handle`**, traits, and `pub` / `mut` — are **supported on the compile path** (JIT and `--emit-exe`). Remaining gaps are listed in [Known limitations](../compiler/known-limitations.md).
...
2. **Post v0.1** — Threaded `@parallel` on the compile path, GPU/SIMD `@vectorize`, Python library interop.
